...
<head>
  ...
  <title>Matricula de Aluno em Turma</title>
</head>
...
<body>
  ...
  <div class="container">
    ...
    <div class="row">
      ...
      <div class="col-md-12">
        <p class="text-center">Selecione o aluno e a turma:</p>
        <fieldset>

          <legend class="text-center">Cadastrar Aluno na Turma</legend>

          <form action="manter/manter_aluno_turma.php" method="post">

            <div class="form-group">
              <label for="id_aluno">Selecione o Aluno(a)</label>
              <select class="form-select" aria-label="Default select example" id="id_matricula" name="id_matricula">
                <option selected>Selecione...</option>
                <?php
                require('manter/conexao.php');

                $consulta = "SELECT aluno.cpf, aluno.nome AS nome, matricula.id_matricula AS matricula FROM aluno INNER JOIN matricula 
            ...
            <div class="form-group">
              <label for="id_professor">Selecione a Turma(disciplina - professor):</label>
              <select class="form-select" aria-label="Default select example" id="id_turma" name="id_turma">
                <option selected>Selecione:</option>
                <?php
                require('manter/conexao.php');

                $consulta = "SELECT turma.id_turma AS cod_turma, turma.id_professor, professor.cpf, professor.nome AS p_nome, curso.coordenador AS coordenador, curso.nome AS nome_curso FROM turma INNER JOIN (professor,curso) ON (turma.id_professor=professor.cpf AND professor.cpf=curso.coordenador)";
                $sql = mysqli_query($conexao, $consulta);
                while ($linha = mysqli_fetch_array($sql)) {
                  echo '<option value="' . $linha['cod_turma'] . '">' . "Turma: " . $linha['cod_turma'] . " - " . $linha['nome_curso'] . " - Coordenador: " . $linha['p_nome'] . '</option>';
                }
                ?>
